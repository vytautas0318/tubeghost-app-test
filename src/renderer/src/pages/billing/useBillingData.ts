...
import { pfPrice, SEAT_RATE, STARTER_BASE } from '@shared/pricing'
...
/**
 * Monthly list price to display for the current subscription.
 *
    ...
    Promise.all([
      ...
      .then(([planResp, profileCount, memberCount, proxyCount, wsResp]) => {
        ...
        setData({
          ...
          // derive it the same way the pricing page does. Stripe remains the
          // authority on what is actually charged — this is display only.
          planPrice: derivePrice(plan, p?.monthly_price_usd ?? null, purchased, w?.extra_seats ?? 0),
          profileLimit,
          seatLimit,
          profileCount,
          memberCount,
