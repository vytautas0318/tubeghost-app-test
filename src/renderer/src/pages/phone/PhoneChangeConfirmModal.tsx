// Confirm step for an in-app PHONE quantity/cycle change.
//
// Modelled on tubeproxies-dash's QuantityChangeModal —
...
import type { PhoneChangePreview } from './phoneQuantityChange'
...
export function PhoneChangeConfirmModal({
  quantity,
  cycle,
  preview,
  loading,
  ...
  const needsSelection = preview?.requiresNumberSelection ?? false
  ...
  const confirmLabel = (() => {
    ...
    if (isDowngrade) return needsSelection ? 'Continue' : 'Schedule change'
  ...
  const numbers = (n: number): string => `${n} ${n === 1 ? 'number' : 'numbers'}`
  const title =
    isIntervalOnly && preview
      ? `${isDowngrade ? 'Downgrade' : 'Upgrade'} to ${periodLabel(cycle)} billing`
      : `${isDowngrade ? 'Downgrade' : 'Upgrade'} to ${numbers(quantity)}`
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          {preview && !isIntervalOnly && (
            <p className="text-xs text-[var(--t2)]">
              {preview.quantityDelta < 0 ? (
                <>You will release {numbers(Math.abs(preview.quantityDelta))}.</>
              ) : preview.quantityDelta > 0 ? (
                <>
                  You will get {numbers(preview.quantityDelta)} more. Your existing{' '}
                  {numbers(preview.current.quantity)} stay the same.
                </>
              ) : null}
            </p>
          )}
          {preview && isIntervalOnly && (
            <p className="text-xs text-[var(--t2)]">
              Same {numbers(preview.next.quantity)} — only the billing period changes.
            </p>
          )}
